...
def main():
    ...
    for idx in tqdm(range(0,len(dataset),min_window_size)):
        ...
        min_obs, _ = torch.min(obs, dim=0)
        max_obs, _ = torch.max(obs, dim=0)
        min_action, _ = torch.min(action, dim=0)
        max_action, _ = torch.max(action, dim=0)

        global_min_obs = torch.min(global_min_obs, min_obs)
        global_max_obs = torch.max(global_max_obs, max_obs)
        global_min_action = torch.min(global_min_action, min_action)
        global_max_action = torch.max(global_max_action, max_action)
    ...
    np.savez('/satassdscratch/scml-shared/calvin_data/task_D_D/stats.npz', **stats)
    # print(n_obs.shape)
